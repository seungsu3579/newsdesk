...
def sent_graph(
    ...
    if vocab_to_idx is None:
        ...
        idx_to_vocab = [
            vocab for vocab, _ in sorted(vocab_to_idx.items(), key=lambda x: x[1])
        ]
...
def numpy_cosine_similarity_matrix(x, min_sim=0.3, verbose=True, batch_size=1000):
    ...
    for bidx in range(math.ceil(n_rows / batch_size)):
        ...
        if verbose:
            print(
                "\rcalculating cosine sentence similarity {} / {}".format(b, n_rows),
                end="",
            )
    mat = sp.sparse.vstack(mat)
    if verbose:
        print(
            "\rcalculating cosine sentence similarity was done with {} sents".format(
                n_rows
            )
        )
    return mat
...
def numpy_textrank_similarity_matrix(
    x, min_sim=0.3, verbose=True, min_length=1, batch_size=1000
):
    ...
    for bidx in range(math.ceil(n_rows / batch_size)):
        ...
        if verbose:
            print(
                "\rcalculating textrank sentence similarity {} / {}".format(b, n_rows),
                end="",
            )
    ...
    if verbose:
        print(
            "\rcalculating textrank sentence similarity was done with {} sents".format(
                n_rows
            )
        )
...
def graph_with_python_sim(tokens, verbose, similarity, min_sim):
    ...
    for i, tokens_i in enumerate(tokens):
        if verbose and i % 1000 == 0:
            print(
                "\rconstructing sentence graph {} / {} ...".format(i, n_sents), end=""
            )
    ...
    if verbose:
        print(
            "\rconstructing sentence graph was constructed from {} sents".format(
                n_sents
            )
        )
